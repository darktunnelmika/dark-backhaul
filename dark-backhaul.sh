...
header() {
  ...
  row "$(printf '%s╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝%s' "$D" "$N")"
  row "$(printf '%sB%s A%s C%s K%s H%s A U L%s   %sreverse tunnel%s' \
...
gh_asset_url() {
  ...
  json="$(curl -fsSL --max-time 20 "https://api.github.com/repos/$GH_REPO/releases/latest" 2>/dev/null)"
  [ -z "$json" ] && return 1
...
screen_core() {
  ...
  case "$KEY" in
    1) info "querying github"
       ...
       curl -fL --retry 3 --max-time 240 -o /tmp/backhaul.pkg "$url" || { bad "download failed"; pause; return; }
       cp -f /tmp/backhaul.pkg "$LOCAL_CORE_DIR/${url##*/}" 2>/dev/null
       mv /tmp/backhaul.pkg "/tmp/${url##*/}"
       if place_core "/tmp/${url##*/}"; then
    ...
    3) ask "direct url"; [ -z "$ANS" ] && return
       curl -fL --retry 3 --max-time 240 -o "/tmp/bh-$(basename "$ANS")" "$ANS" || { bad "download failed"; pause; return; }
       if place_core "/tmp/bh-$(basename "$ANS")"; then
         echo custom > "$CORE_VER_FILE"; ensure_units
         ok "installed: $(core_version_short)"; restart_all_prompt
       fi
       rm -f "/tmp/bh-$(basename "$ANS")" ;;
    *) return ;;
  esac
...
EOF
  cat > "$RS_UNIT" <<EOF
[Unit]
Description=DARK VPN Backhaul scheduled restart (%i)
...
EOF
  cat > "$RS_TIMER" <<'EOF'
[Unit]
Description=DARK VPN Backhaul scheduled restart timer (%i)
...
issue_cert() { # <reload-service-or-empty>
  ...
  if ! "$(acme_bin)" --issue -d "$d" --standalone --server letsencrypt \
        --keylength ec-256 >/tmp/acme.$$.log 2>&1; then
    bad "issuance failed"
    tail -n 8 /tmp/acme.$$.log | sed 's/^/    /'
    rm -f /tmp/acme.$$.log
    return 1
  fi
  rm -f /tmp/acme.$$.log
...
write_server_config() {
  ...
  tune_effective
  {
    echo "# DARK VPN Backhaul | IRAN (server) | $DEV_ID"
...
write_client_config() {
  ...
  tune_effective
  {
    echo "# DARK VPN Backhaul | KHAREJ (client) | $DEV_ID"
    echo "# $(date '+%F %T') | profile: ${PROFILE:-balanced}"
    echo "[client]"
...
# B2|IRAN_IP|PORT|TOKEN|TRANSPORT#|PROFILE|RESTART|PORTS   (B1 kept for old codes)
make_pair_code() {
  local p="B2|$PUB_IP|$PORT|$TOKEN|$(tr_idx "$TRANSPORT")|${PROFILE:-balanced}|${RESTART_EVERY:-off}|$(ports_csv "$1")"
  printf 'DBH-%s' "$(printf '%s' "$p" | b64enc)"
}
parse_pair_code() {
  local code="$1" raw ver ti
  code="${code#DBH-}"; code="$(tr -d '[:space:]' <<<"$code")"
  ...
  valid_host "$PC_IP" && valid_port "$PC_PORT" && [ -n "$PC_TOKEN" ]
}
show_pair_code() {
...
screen_new_kharej() {
  ...
  if ! parse_pair_code "$ANS"; then
    ...
    valid_host "$PC_IP" && valid_port "$PC_PORT" && [ -n "$PC_TOKEN" ] || { bad "invalid input"; pause; return; }
  fi
  ...
  case "$PC_TR" in ws|wss|wsmux|wssmux)
    if yesno "connect through a cdn edge ip?" n; then
      ask "edge ip" "188.114.96.0"; EDGE_IP="$ANS"
    fi ;;
  esac
...
screen_manage() {
  ...
  while :; do
    ...
    mid; sect "INSPECT"
    item 8 "Show config" ""
    item p "Pair code" "iran side only"
    item e "Edit config by hand" ""
...
screen_dashboard() {
  ...
  while :; do
    ...
    mid; sect "LISTENING"
    local socks ln
    socks="$(ss -tulnp 2>/dev/null | grep -i backhaul | awk '{print $1"  "$5}' | head -n 8)"
...
screen_update() {
  while :; do
    ...
    item 4 "Install as command" "run as: darkbh"
    item 0 "Back" ""
    bot; echo; getkey
    case "$KEY" in
      1) screen_core ;;
      ...
      4) install -m 0755 "$SELF_PATH" /usr/local/bin/darkbh && ok "run: darkbh" || bad "failed"; pause ;;
      0|_) return ;;
    esac
  done
}
update_script() {
  ...
  grep -q 'DARKVPN-BACKHAUL-SCRIPT' "$tmp" || { bad "not a Backhaul manager script - aborted"; rm -f "$tmp"; return; }
...
screen_uninstall() {
  ...
  rm -f "$UNIT_FILE" "$RS_UNIT" "$RS_TIMER"; systemctl daemon-reload 2>/dev/null
...
main_menu() {
  while :; do
    ...
    case "$KEY" in
      ...
      0|q|Q) clear; printf '  %sDARK VPN - Backhaul%s  %s%s%s\n\n' "$C" "$N" "$D" "$DEV_ID" "$N"; exit 0 ;;
    esac
  done
}
